...
> 安装目录可以任意选择（包括 `D:\Apps` 这类用户可写位置）：提权执行端部署在系统受保护位置 `C:\ProgramData\XianTrace\UACToolBox`，计划任务、环境变量与右键菜单都指向它；配置界面经注册表记录的文件哈希核验，被替换即拒绝保存。不会修改用户目录的任何权限。

## 安全模型

- 提权执行端 `Launcher.exe` 部署并校验于受保护的系统位置（`ProgramData\XianTrace\UACToolBox`）：所有者为管理员，普通用户不可写入，与安装位置无关。
- 配置文件带所有者校验与修订号乐观锁，仅管理员授权可写；保存请求的调用方按注册表记录的配置界面路径 + SHA-256 核验，界面文件被替换即拒绝。
- 配置文件位于 `C:\ProgramData\XianTrace\UACToolBox`（所有者管理员、普通用户只读），带修订号乐观锁。
